...
public class BlogDao implements AutoCloseable {
        ...
        try(PreparedStatement stmt = con.prepareStatement(sql)) {
            try(ResultSet rs = stmt.executeQuery()) {
                while(rs.next()) {
                    ...
                    Timestamp created = rs.getTimestamp("createdOn");
    ...
    public int save(Blog b) throws Exception {
        String sql = "INSERT INTO blogs(title,contents,userId,categoryId,createdOn) VALUES(?,?,?,?,NOW())";
        try(PreparedStatement stmt = con.prepareStatement(sql)) {
    ...
    public int update(Blog b) throws Exception {
        String sql = "UPDATE blogs SET title=?, contents=?, userId=?, category_id=? WHERE id=?";
        try(PreparedStatement stmt = con.prepareStatement(sql)) {
            stmt.setString(1, b.getTitle());
            stmt.setString(2, b.getContents());
            stmt.setInt(3, b.getUserId());
